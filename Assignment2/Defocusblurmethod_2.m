...
%% Designing the gaussian filter
sizeofilter = 11; %size of gaussian filter
sigmaoffilter = 1; %sigma of gaussian filter
filt = fspecial('gaussian', sizeofilter, sigmaoffilter) ; %Creating gaussian filter since gaussian filter produces defocus effect
%%Loading the image
...
I = xnorm;
figure(),imshow(xnorm)
title('Blurredimagedefocuswithnoisedefocus')
...
title('DeconvolvedimagewithNoisedefocus')
drawnow
...
figure()
imshow(xnorm)
title('Blurredimagedefocuswithoutnoisedefocus')
...
Answer = (x((3*(hfs_y1))+1:n-(3*(hfs_y1)),(3*(hfs_x1))+1:m-(3*(hfs_y1)))); %Our final deconvolved image
figure, imshow(Answer)
title('DeconvolvedimagewithoutNoisedefocus')
drawnow
